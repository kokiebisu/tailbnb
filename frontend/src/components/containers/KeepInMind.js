import React from 'react';
export default () => {
  return (
    <>
      <div className='mt-12 mb-4'>
        <div className='py-4 border-b border-gray-300'>
          <h3
            style={{ fontFamily: 'airbnb-bold' }}
            className='text-gray-750 text-2xl'>
            Things to keep in mind
          </h3>
          <div className='pt-6'>
            <p className='text-gray-750'>
              <span style={{ fontFamily: 'airbnb-medium' }}>Check-in:</span>{' '}
              After 4:00 p.m.
            </p>
            <p className='text-gray-750'>
              <span style={{ fontFamily: 'airbnb-medium' }}>Checkout:</span>{' '}
              12:00 p.m.
            </p>
          </div>
        </div>
        <div className='py-6 border-b border-gray-300'>
          <h3 className='text-gray-750' style={{ fontFamily: 'airbnb-bold' }}>
            House Rules
          </h3>
          <div className='flex items-start justify-start flex-wrap my-4'>
            <div className='flex item-center justify-start w-1/2 py-2 bg-blue'>
              <div>
                <svg
                  className='h-5 w-5'
                  style={{ fill: '#484848' }}
                  xmlns='http://www.w3.org/2000/svg'
                  viewBox='0 0 512 512'>
                  <g>
                    <g>
                      <g>
                        <path
                          d='M437.333,132.267c12.8,12.8,17.067,17.067-10.667,44.8l29.867,29.867c23.467-23.467,34.133-44.8,34.133-64
				c-2.42-15.991-10.804-30.473-23.467-40.533c-12.8-12.8-17.067-17.067,10.667-44.8L448,27.733
				c-23.467,23.467-34.133,44.8-34.133,64C416.286,107.724,424.671,122.206,437.333,132.267z'
                        />
                        <path
                          d='M345.6,132.267c12.8,12.8,17.067,17.067-10.667,44.8l29.867,29.867c23.467-23.467,34.133-44.8,34.133-64
				c-2.42-15.991-10.804-30.473-23.467-40.533c-12.8-12.8-17.067-17.067,10.667-44.8l-29.867-29.867
				c-23.467,23.467-34.133,44.8-34.133,64C324.553,107.724,332.937,122.206,345.6,132.267z'
                        />
                        <path
                          d='M245.333,132.267c12.8,12.8,17.067,17.067-10.667,44.8l29.867,29.867c23.467-23.467,34.133-44.8,34.133-64
				c-2.42-15.991-10.804-30.473-23.467-40.533c-12.8-12.8-17.067-17.067,10.667-44.8L256,27.733
				c-23.467,23.467-34.133,44.8-34.133,64C224.286,107.724,232.671,122.206,245.333,132.267z'
                        />
                        <path
                          d='M490.667,256H21.333C9.551,256,0,265.551,0,277.333v128c0,11.782,9.551,21.333,21.333,21.333h238.933l21.333-21.333
				L260.267,384H128v-85.333h341.333V384H422.4l-21.333,21.333l21.333,21.333h68.267c11.782,0,21.333-9.551,21.333-21.333v-128
				C512,265.551,502.449,256,490.667,256z'
                        />
                        <polygon
                          points='420.267,356.267 390.4,326.4 341.333,375.467 292.267,326.4 262.4,356.267 311.467,405.333 262.4,454.4 
				292.267,484.267 341.333,435.2 390.4,484.267 420.267,454.4 371.2,405.333 			'
                        />
                      </g>
                    </g>
                  </g>
                </svg>
              </div>
              <p
                style={{ fontFamily: 'airbnb-book' }}
                className='text-gray-750 pl-4'>
                No smoking
              </p>
            </div>
            <div className='flex item-center justify-start w-1/2 py-2 bg-blue'>
              <div>
                <svg
                  className='h-5 w-5'
                  style={{ fill: '#484848', marginTop: 2 }}
                  xmlns='http://www.w3.org/2000/svg'
                  viewBox='0 0 512 512'>
                  <g>
                    <g>
                      <g>
                        <path
                          d='M180.365,289.615c7.521-7.531,11.646-18.938,11.635-32.094c0-4.677-0.531-9.531-1.583-14.427
				C184.26,214.438,162.198,192,140.177,192c-10.125,0-19.083,4.615-25.229,12.99c-5.427,7.385-8.292,17.125-8.281,28.156
				c0,4.667,0.531,9.51,1.583,14.417v0.01c6.156,28.656,28.219,51.094,50.24,51.094
				C166.885,298.667,174.448,295.531,180.365,289.615z'
                        />
                        <path
                          d='M371.823,192c-22.021,0-44.083,22.438-50.24,51.094c-1.052,4.896-1.583,9.75-1.583,14.417
				c-0.01,13.167,4.115,24.573,11.635,32.104c5.917,5.917,13.479,9.052,21.875,9.052c22.021,0,44.083-22.438,50.24-51.094v-0.01
				c1.052-4.906,1.583-9.75,1.583-14.406c0.01-11.042-2.854-20.781-8.281-28.167C390.906,196.615,381.948,192,371.823,192z'
                        />
                        <path
                          d='M350.76,161.99v-0.01c0.729-3.854,1.125-7.594,1.219-11.188c0.354-14.063-3.823-26.313-11.781-34.479
				c-6.156-6.313-14.021-9.646-22.76-9.646c-22.302,0-44.063,22.552-49.531,51.354c-0.729,3.854-1.125,7.594-1.219,11.188
				c-0.354,14.063,3.823,26.313,11.781,34.479c6.156,6.313,14.021,9.646,22.76,9.646C323.531,213.333,345.292,190.781,350.76,161.99
				z'
                        />
                        <path
                          d='M256,0C114.844,0,0,114.844,0,256s114.844,256,256,256s256-114.844,256-256S397.156,0,256,0z M256,469.333
				c-117.635,0-213.333-95.698-213.333-213.333c0-51.151,18.125-98.126,48.249-134.918l128.119,128.118
				c-9.776,9.96-14.715,22.969-18.19,32.57c-1.531,4.219-3.427,9.479-4.74,11.042c-4.365,5.26-9.729,10.135-15.417,15.292
				c-14.531,13.177-31,28.115-31.354,52.479c-0.177,12.896,3.802,23.854,11.521,31.688c8.427,8.542,20.906,13.063,36.073,13.063
				c17.073,0,27.823-5.677,36.458-10.24c7.396-3.906,13.229-6.99,22.604-6.99c9.385,0,15.229,3.083,22.625,6.99
				c8.635,4.563,19.396,10.24,36.458,10.24c15.167,0,27.646-4.521,36.073-13.063c1.835-1.862,3.426-3.927,4.832-6.126l34.94,34.94
				C354.126,451.208,307.151,469.333,256,469.333z M421.085,390.918L310.693,280.526c-5.811-16.014-15.572-40.298-44.879-44.879
				l-32.142-32.142c7.854-8.167,11.992-20.328,11.641-34.276c-0.094-3.615-0.49-7.354-1.219-11.219
				c-5.469-28.792-27.229-51.344-49.531-51.344c-8.74,0-16.604,3.333-22.76,9.646c-4.203,4.313-7.273,9.815-9.255,16.068
				l-41.465-41.465C157.874,60.792,204.849,42.667,256,42.667c117.635,0,213.333,95.698,213.333,213.333
				C469.333,307.151,451.208,354.126,421.085,390.918z'
                        />
                      </g>
                    </g>
                  </g>
                </svg>
              </div>
              <p
                style={{ fontFamily: 'airbnb-book' }}
                className='text-gray-750 pl-4'>
                No pets
              </p>
            </div>
            <div className='flex item-center justify-start w-1/2 py-2 bg-blue'>
              <div>
                <svg
                  style={{ fill: '#484848' }}
                  className='h-5 w-5'
                  viewBox='0 0 512 512'
                  xmlns='http://www.w3.org/2000/svg'>
                  <g>
                    <path d='m331 242c-29.021 0-55.931 9.206-77.967 24.849-7.349-33.545-27.97-63.113-57.033-80.693v-171.156c0-8.284-6.716-15-15-15h-60c-8.284 0-15 6.716-15 15v171.156c-36.731 22.219-60 63.58-60 107.926v202.918c0 8.284 6.716 15 15 15h180c8.284 0 15-6.716 15-15v-7.806c21.466 14.397 47.268 22.806 75 22.806 74.439 0 135-60.561 135-135s-60.561-135-135-135zm-255 89.99h127.722c-4.994 14.092-7.722 29.238-7.722 45.01s2.728 30.918 7.722 45h-127.722zm60-241.99h30v30h-30zm30-60v30h-30v-30zm-38.292 178.417c5.082-2.542 8.292-7.735 8.292-13.417v-45h30v45c0 5.682 3.21 10.875 8.292 13.417 30.412 15.206 50.996 48.747 51.676 83.875-2.526 3.126-4.918 6.363-7.162 9.708h-142.806v-7.918c0-35.775 20.78-70.202 51.708-85.665zm-51.708 273.583v-30.01h142.806c2.253 3.37 4.656 6.62 7.194 9.758v20.252zm255 0c-57.897 0-105-47.103-105-105s47.103-105 105-105 105 47.103 105 105-47.103 105-105 105z' />
                    <path d='m384.033 323.967c-5.857-5.858-15.355-5.858-21.213 0l-31.82 31.82-31.82-31.82c-5.857-5.858-15.355-5.858-21.213 0s-5.858 15.355 0 21.213l31.82 31.82-31.82 31.82c-5.858 5.858-5.858 15.355 0 21.213 5.857 5.858 15.355 5.859 21.213 0l31.82-31.82 31.82 31.82c5.857 5.858 15.355 5.859 21.213 0 5.858-5.858 5.858-15.355 0-21.213l-31.82-31.82 31.82-31.82c5.858-5.858 5.858-15.355 0-21.213z' />
                  </g>
                </svg>
              </div>
              <p
                style={{ fontFamily: 'airbnb-book' }}
                className='text-gray-750 pl-4'>
                No parties or events
              </p>
            </div>
          </div>
          <button>
            <div className='flex items-center justify-start'>
              <p
                className='text-green-850 hover:border-green-850 border-b text-green-850 border-transparent'
                style={{ fontFamily: 'airbnb-medium' }}>
                Read all rules
              </p>
              <div className='ml-3'>
                <svg
                  style={{ fill: '#008489' }}
                  viewBox='0 0 18 18'
                  className='h-2 w-2'>
                  <path d='m16.29 4.3a1 1 0 1 1 1.41 1.42l-8 8a1 1 0 0 1 -1.41 0l-8-8a1 1 0 1 1 1.41-1.42l7.29 7.29z' />
                </svg>
              </div>
            </div>
          </button>
        </div>
      </div>
    </>
  );
};
